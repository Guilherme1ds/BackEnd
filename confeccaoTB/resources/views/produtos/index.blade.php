<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('produtos.create') }}" class="btn btn-primary text-xs uppercase tracking-widest">
                + Novo Produto
            </a>
        </div>
    </x-slot>
                        ...
                        <div class="card p-5 hover:shadow-xl transition">
                            <div>
                                ...
                                <p class="text-sm text-indigo-600 font-medium">
                                    <span class="font-semibold text-gray-800">Preço:</span> R$ {{ number_format($produto->preco, 2, ',', '.') }}
                                </p>
                                <p class="text-sm text-gray-600">
                                    <span class="font-semibold text-gray-800">Estoque:</span> {{ $produto->quantidade_estoque }}
                                </p>
                            </div>

                            <div class="flex items-center justify-end mt-6 pt-4 border-t border-gray-200 space-x-4">
                                <a href="{{ route('produtos.edit', $produto->id) }}" class="btn btn-secondary text-sm">
                                    Editar
                                </a>
                                ...
                                <form action="{{ route('produtos.destroy', $produto->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" class="btn btn-danger text-sm font-semibold btn-excluir" data-item-name="{{ $produto->nome }}">
                                        Excluir
                                    </button>
                                </form>
                            </div>
